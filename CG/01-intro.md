...
להרחבה על האפליקציות השונות והתפקיד שמשחקת בהן הגיאומטריה החישובית, מומלץ לקרוא את סעיף 1.3 בספר.
...
<img src="images/1/line.jpg" align="left" width="42%"/> **ישר (line)** $\ell$ הוא תת-מרחב חד-ממדי, וניתן לתאר אותו על ידי זוג נקודות $p$ ו-$q$, בתור אוסף הנקודות שמקיימות את המשוואה $\ell=(1-\lambda)p+\lambda q$ עבור $\lambda\in\mathbb{R}$ כלשהו.
...
**על-מישור (hyperplane)** $h$ הוא תת-מרחב אוקלידי מממד $(d-1)$ בתוך מרחב אוקלידי מממד $d$. למשל, במרחב הדו-ממדי אלו **ישרים**, ובמרחב התלת-ממדי אלו **מישורים**. על-מישור חוצה את המרחב לשני חצאי מרחבים -- כל חצי מרחב כזה נקרא **חצי מישור (halfplane)**.
...
<img src="images/1/disk.jpg" align="left" width="25%"/> במישור, **מעגל (circle)** מוגדר על ידי **מרכז** (נקודה $o$) ו**רדיוס** (סקלר $r$), והוא המיקום הגיאומטרי של כל הנקודות שהמרחק שלהן מ-$o$ הוא בדיוק $r$. כלומר עבור $o=(a,b)$, אלו כל הנקודות המקיימות את המשוואה $(x-a)^2+(y-b)^2=r^2$.
...
העקום $C$ נקרא השפה של $P$ (ומסומן גם $\partial P$), ואילו קבוצת הנקודות בפוליגון $P$ שאינן על $C$ נקראת הפנים של $P$ (ומסומנת $\text{int}(P)$). באיור, פנים הפוליגון מוצלל באפור.
:::
